...
using static CQC.ConTest.Loading;
...
namespace CQC.ConTest
{
    public partial class frmMain : DevExpress.XtraBars.ToolbarForm.ToolbarForm
    {
        ...
        CustomOverlayPainter m_CustomOverlayPainter = null;
        ...
        private Thread m_ExectueThread = null;                  // 执行线程
        ...
        public frmMain()
        {
            try
            {
                ...
                splashScreenManager1 = new DevExpress.XtraSplashScreen.SplashScreenManager(this, typeof(Loading), false, false);
        ...
        void startTestExec()
        {
            ...
            m_CustomOverlayPainter = new CustomOverlayPainter(this);
            m_CustomOverlayPainter.CancelWaitingEvent += CancelWaiting;
            m_CustomOverlayPainter.Caption = "Test Executing";
            m_CustomOverlayPainter.Text = "Executing Test Schedule，Please Waiting.....";

            loadHandle = SplashScreenManager.ShowOverlayForm(this, true, true, null, null, 100,
                customPainter: new OverlayWindowCompositePainter(m_CustomOverlayPainter),
                animationType: WaitAnimationType.Line, lineAnimationParameters: new LineAnimationParams(10, 8, 3));
        ...
        {
            if (null != m_ExectueThread && true == m_ExectueThread.IsAlive)
            {
                m_ExectueThread.Abort();
            }
        }

        void endTestExec()
        {
            if (true == m_CustomOverlayPainter.HasCanceled)
            {
                m_CustomOverlayPainter.Caption = "Test Canceling";
                m_CustomOverlayPainter.Text = "Test Canceling....";
                sendOutput(2, "========== Test Canceled ==========");
            }
            else
            {
                m_CustomOverlayPainter.Caption = "Test Ending";
                m_CustomOverlayPainter.Text = "Test Ending....";
        ...
        private void ExecuteTesting(object paramValue)
        {
            if (null == paramValue)
            {
                return;
            ...
            if (true == paramValue is Tuple<Action>)
            {
                executeTestSchedule((paramValue as Tuple<Action>).Item1);
            }
            else if (true == paramValue is Tuple<TestClassModel, Action>)
            {
                executeTestSchedule(
                    (paramValue as Tuple<TestClassModel, Action>).Item1,
                    (paramValue as Tuple<TestClassModel, Action>).Item2);
            }
            else if (true == paramValue is Tuple<TestCaseModel, Action>)
            {
                executeTestSchedule(
                    (paramValue as Tuple<TestCaseModel, Action>).Item1,
                    (paramValue as Tuple<TestCaseModel, Action>).Item2);
            }
        }

        private void executeTestSchedule(object callBackEvent)
        {
            try
            {
        ...
        private void executeTestSchedule(TestClassModel classModel, object callBackEvent)
        {
            try
            {
                ...
                try
                {
                    foreach (testClass tcl in testToExec)
                    {
                        if (false == tcl.name.Equals(classModel.GetFinalName()))
                        {
                            return;
                        }
        ...
        private void executeTestSchedule(TestCaseModel caseModel, object callBackEvent)
        {
            try
            {
                bTesting = true;
                ...
                try
                {
                    foreach (testClass tcl in testToExec)
                    {
                        if (false == tcl.name.Equals(caseModel.ParentNodedata.GetFinalName()))
                        {
                            return;
                        }
                        ...
                        {
                            if (false == tc.name.Equals(caseModel.GetFinalName()))
                            {
                                return;
                            }
        ...
        private void ExecuteTestClass(testClass tcl)
        {
            try
            {
        ...
        private void ExecuteTestcase(testCase tc)
        {
            try
            {
                numofcase++;
        ...
        private void SaveLogAndReport()
        {
            try
            {
                ...
                if (true == tp.TestReport)
                {
                    m_CustomOverlayPainter.Caption = "Test Reporting";
                    m_CustomOverlayPainter.Text = "Gernerating Test Report....";
        ...
        private void ExecuteTest()
        {
            ...
            {
                startTestExec();
                m_ExectueThread = new Thread(ExecuteTesting);
                m_ExectueThread.Start(new Tuple<Action>(new Action(endTestExec)));
        ...
        /// 执行测试
        /// </summary>
        private void ExecuteTest(TestClassModel classModel)
        {
            try
            {
                startTestExec();
                m_ExectueThread = new Thread(ExecuteTesting);
                m_ExectueThread.Start(new Tuple<TestClassModel, Action>(classModel, new Action(endTestExec)));
        ...
        /// 执行测试
        /// </summary>
        private void ExecuteTest(TestCaseModel caseModel)
        {
            try
            {
                startTestExec();
                m_ExectueThread = new Thread(ExecuteTesting);
                m_ExectueThread.Start(new Tuple<TestCaseModel, Action>(caseModel, new Action(endTestExec)));
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        */
        ...
        void executeDPTestcase(testCase tc)
        {
            ...
            {
                //MessageBox.Show(exp.ToString());
                string exStr = exp.Message;
                sendOutput(2, exStr);
                saveLogfile(sw, exStr);
                casefailed++;
                sw.Flush();
                sw.Close();
                fs.Close();
        ...
        /// <param name="e"></param>
        private void siExcClass_ItemClick(object sender, ItemClickEventArgs e)
        {
            TreeListNode node = testProjectExplorer.TreeView.FocusedNode;
            if (null == node || false == node.Tag is NodeInfo)
            {
                return;
            }
            ...
            if (NodeType.testclass == nodeInfo.ntype)
            {
                ExecuteTest(nodeInfo.SourceData as TestClassModel);
            }
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void siEdit_ItemClick(object sender, ItemClickEventArgs e)
        {
         
        }

        private void siExcCase_ItemClick(object sender, ItemClickEventArgs e)
        {
            TreeListNode node = testProjectExplorer.TreeView.FocusedNode;
            if (null == node || false == node.Tag is NodeInfo)
            {
            ...
            if (NodeType.testcase == nodeInfo.ntype)
            {
                ExecuteTest(nodeInfo.SourceData as TestCaseModel);
            }
        }

        private void testProjectExplorer_ExecuteTestCaseEvent(TestCaseModel obj)
        {
            ExecuteTest(obj);
        }

        private void testProjectExplorer_ExecuteTestClassEvent(TestClassModel obj)
        {
            ExecuteTest(obj);
        }

        private void siExcSch_ItemClick(object sender, ItemClickEventArgs e)
        {
            ExecuteTest();
        }

        private void popupMenu2_BeforePopup(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                TreeListNode node = testProjectExplorer.TreeView.FocusedNode;
                if (null == node || false == node.Tag is NodeInfo)
                {
                    siExcClass.Enabled = false;
                    siExcCase.Enabled = false;
                }
                ...
                {
                    siExcClass.Enabled = false;
                    siExcCase.Enabled = true;
                }
                else if (NodeType.testclass == nodeInfo.ntype)
                {
                    siExcClass.Enabled = true;
                    siExcCase.Enabled = false;
                }
                else
                {
